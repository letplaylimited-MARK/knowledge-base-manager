...
class NamingOptimizer:
    ...
    def _generate_reason(self, issues: List[Dict], suggested: str) -> str:
        ...
        reasons = {
            'too_simple': f"原命名过于简单，建议改为描述性命名",
            'generic': f"原命名过于通用，建议体现具体内容",
            'missing_quality': f"内容质量高但命名未体现，建议添加质量标记",
            'missing_type': f"命名缺少内容类型标识",
            'format_chaos': f"命名格式不统一，建议标准化",
            'too_long': f"原命名过长，建议精简",
        }
