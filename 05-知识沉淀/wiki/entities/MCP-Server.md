...
17. `get_graph` — MemoryOS 摘要查询
18. `watch_inbox` — 收件箱监控
19. `get_content_stats` — 内容统计
20. `enhanced_scan_inbox` — 增强收件箱扫描

> 当前实现说明：`keyword_search` 调用 `search_content.py` 做文件扫描，不是 SQLite FTS5；`get_graph` 返回 MemoryOS 运行摘要，不是完整知识图谱查询。
